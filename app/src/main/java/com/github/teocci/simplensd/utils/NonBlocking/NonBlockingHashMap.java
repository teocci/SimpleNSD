package com.github.teocci.simplensd.utils.NonBlocking;
...
public class NonBlockingHashMap<TypeK, TypeV>
    ...
    private static final int REPROVE_LIMIT = 10; // Too many reproves then force a table-resize

    // --- Bits to allow Unsafe access to arrays
    private static final UnsafeAndroid _unsafe = new UnsafeAndroid();
    ...
    // Count of reproves
    private transient Counter _reproves = new Counter();
    /** Get and clear the current count of reproves.  Reproves happen on key
     *  collisions, and a high reprove rate may indicate a poor hash function or
     *  weaknesses in the table resizing function.
     *  @return the count of reproves since the last call to {@link #reproves}
     *  or since the table was created.   */
    public long reproves() { long r = _reproves.get(); _reproves = new Counter(); return r; }
    ...
    private static final Object get_impl( final NonBlockingHashMap topmap, final Object[] kvs, final Object key, final int fullhash ) {
        ...
        final int[] hashes=hashes(kvs); // The memorized hashes; reads slot 1 of kvs

        int idx = fullhash & (len-1); // First key hash

        // Main spin/reprove loop, looking for a Key hit
        int reprobe_cnt=0;
        while( true ) {
            ...
            // needs to force a table-resize for a too-long key-reprove sequence.
            // Check for too-many-reproves on get - and flip to the new table.
            ...
            idx = (idx+1)&(len-1);    // Reprove by 1!  (could now prefetch)
        }
    }
        ...
        while( true ) {             // Spin till we get a Key slot
            ...
            if( K == null ) {         // Slot is free?
                ...
                if( CAS_key(kvs,idx, null, key ) ) { // Claim slot for Key
                    ...
                    hashes[idx] = fullhash; // Memorize fullhash
                    break;                  // Got it!
                }
                ...
                assert K != null;       // If keys[idx] is null, CAS should work
            }
    ...
    private static final class CHM<TypeK,TypeV> {
        ...
        // end up reproving too much.  Last-resize-milli is the time since the
        ...
        // new table.  Note that if a 'get' call has reproved too many times and
        ...
        // table because of too many reproves.  The invariant is:
        //   slots.estimate_sum >= max_reprove_cnt >= reprove_limit(len)
        private final boolean tableFull( int reprove_cnt, int len ) {
            return
                    // Do the cheap check first: we allow some number of reproves always
                    reprove_cnt >= REPROVE_LIMIT &&
        ...
        private final Object[] resize( NonBlockingHashMap topmap, Object[] kvs) {
            ...
            // and we need some decent padding to avoid endless reproving.
